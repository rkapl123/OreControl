...
open SQLOUT, ">Data/covariancedata.sql";
print SQLOUT "use ORE\n\n";
################################
# process Covariancedata
if (-e $covarianceFile) {
	print "processing $covarianceFile\n";
	open (FH_MD, "<".$covarianceFile);
	my $linenum;
	while (<FH_MD>) {
		$linenum++;
		chomp;
		next if /^#/ or $_ eq "";
		my ($Quotestring1,$Quotestring2,$QuoteValue) = /^(.*?)\s+(.*?)\s+(.*?)$/;
		...
		print SQLOUT "INSERT MdatCovarianceData (QuoteId1,QuoteId2,QuoteValue) VALUES ('".$Quotestring1."','".$Quotestring2."',".$QuoteValue.");\n";
			...
			print $Quotevaluehash{$Quotestring1.$Quotestring2};
		}
	}
	close FH_MD;
}
...
open SQLOUT, ">Data/fixingdata.sql";
print SQLOUT "use ORE\n\n";
################################
# process Fixingdata
if (-e $fixingdataFile) {
...
sub processQuotestring {
	my ($QuoteDate,$Quotestring) = @_;
	my $colNames; my $colValues;
