...
namespace TreeizeRelD {
    ...
    // __declspec(dllexport) needed for BOOST testcases (creates LIB file needed)...
    ...
    __declspec(dllexport) std::string writeTreeAndCreateXML(const std::vector<std::vector<std::string>>& control,
    ...
    __declspec(dllexport) std::string writeTreeAndCreateJSON(const std::vector<std::vector<std::string>>& control,
        const std::vector<std::vector<std::vector<std::string>>>& data, int *result);
    ...
    __declspec(dllexport) std::string writeTree(const std::vector<std::vector<std::string>> &control,
    ...
    __declspec(dllexport) std::string createXML(pt::ptree & ptTree, std::string& resultString);

    // create Json from previously (with writeTree) created  ptTree
    __declspec(dllexport) std::string createJson(pt::ptree & ptTree, std::string& resultString);
    ...
    __declspec(dllexport) std::string writeTable(pt::ptree &ptFKTable, const std::vector<std::vector<std::string>> &table,
    ...
    __declspec(dllexport) std::string writeRecord(pt::ptree &ptRecord, const std::vector<std::string> &recordRow,
